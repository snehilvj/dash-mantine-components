...
- `multi` (Bool; optional): Allow multiple values to be picked
- `noWrap` (Bool; optional): Defined flex-wrap property
- `options` (optional): Chips. options has the following type: Array of lists containing elements 'label', 'value', 'disabled'.
...
function chips(; kwargs...)
        available_props = Symbol[:id, :align, :className, :color, :direction, :grow, :multi, :noWrap, :options, :position, :radius, :size, :spacing, :style, :value, :variant]
        wild_props = Symbol[]

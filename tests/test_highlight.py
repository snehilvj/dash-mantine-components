...
def test_001hi_highlight_component(dash_duo):
    ...
    dash_duo.wait_for_text_to_equal(
        "#highlight-text", "You can change styles of highlighted parts"
    )
    ...
    assert (
        highlight_element.value_of_css_property("background-color")
        == "rgba(0, 0, 255, 1)"
    )  # Blue background
    assert (
        highlight_element.value_of_css_property("color") == "rgba(255, 255, 255, 1)"
    )  # White text
